...
//создать список из уникальных элементов, чтоб не повторялись
// let result = []
// products.forEach(item => {
...
// let result = numbers.map(elem => elem ** 3)
// console.log(result);

// let result2 = products.map(elem => elem.name)
// console.log(result2);

// Из массива товаров сделать новый массив из товаров, у которых будут такие поля:
...
let result3 = products.map(elem => {
    let newPrice = elem.price;
    if (elem.discount !== undefined) {
        newPrice = newPrice * elem.discount
    }
    return {
        productName: elem.name + `, ` + elem.category,
